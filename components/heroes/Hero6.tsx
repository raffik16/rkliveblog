...
interface Hero6Props {
  /** Server-side resolved title config (for SSR/SEO) */
  titleConfig?: HeroTitleVariation
}

// Shared content component that renders the hero content
function HeroContent({ titles, subtitle }: { titles: string[]; subtitle?: string }) {
  return (
...
// Client-side component that uses the UTM hook (fallback when no server props)
function Hero6ClientContent() {
  const { titles, subtitle } = useUTMTitle()
  return <HeroContent titles={titles} subtitle={subtitle} />
}
...
export default function Hero6({ titleConfig }: Hero6Props) {
  return (
    <GradientFlow
    ...
    >
      ...
      <div className="relative z-10 mx-auto w-full max-w-7xl px-4 py-12 sm:px-8 sm:py-16 lg:px-12 lg:py-24">
        ...
        <div className="glass-frost-hero glass-border-hero relative overflow-hidden rounded-[2rem] p-6 shadow-2xl sm:rounded-[2.5rem] sm:p-12 lg:p-16">
          ...
          {/* Inner Content - Use server props if available, otherwise client-side */}
          {titleConfig ? (
            <HeroContent titles={titleConfig.titles} subtitle={titleConfig.subtitle} />
          ) : (
